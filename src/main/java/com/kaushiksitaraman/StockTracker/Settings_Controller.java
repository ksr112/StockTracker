    		...
    		Thread s = new Thread(new Runnable() {
    			...
    			{	log.debug("3.1 Now checking whether the inputted value is legit or not, by calling the ConnStockQuote");
    				try {
						...
						resp_result = connection.get_connection(key);
    				...
    				Platform.runLater(new Runnable() {
    					public void run() {
    						...
    						if(resp_result.contains("subscribed") || resp_result.contains("IOException"))
    			    		...
    			    		{	log.debug("3.1.7 We inside the 'else' part which has the approved handshake code");
    			    			 ...
    			    			 log.debug("3.1.8 Creating a file 'skey.sr' to store the key value");  	
    			    	         ...
    			    	         Thread t = new Thread(new Runnable() {
    									...
    									public void run() {
    										...
    										try {
    											...
    											log.debug("3.1.10.3 Sleeping for 4 seconds :-) ");
    											Thread.sleep(4000);
    										} catch (InterruptedException e) {
    											// TODO Auto-generated catch block
    											//e.printStackTrace();
...
//    											btn_keydelete.setVisible(true);
    ...
    public void start_BSEstage(Rectangle2D primaryScreenBounds, Dimension screenSize, Insets insets, double offset_height) {
        tickerbse_counter = 1;
      ...
      void bsetrackopen(ActionEvent event) throws IOException {
        if (tickerbse_counter == 0) {
          ...
          if (this.chkbox_nse.isSelected() == true) {
            ...
            double calculatestageheightNSE = primaryScreenBounds.getMaxY() - nse_stage.getHeight() - stageheightNSE; //- insets.bottom
            if (calculatestageheightNSE == stagelocationNSE) {
              start_BSEstage(primaryScreenBounds, screenSize, insets, defaultlocationBSE);
            } else {
              start_BSEstage(primaryScreenBounds, screenSize, insets, tempstage.getHeight());
            } 
          } else {
            start_BSEstage(primaryScreenBounds, screenSize, insets, defaultlocationBSE);
          } 
