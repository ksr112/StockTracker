    			    		...
    			    		{	log.debug("3.1.7 We inside the 'else' part which has the approved handshake code");
    			    	         ...
    			    	         Thread t = new Thread(new Runnable() {
    									...
    									public void run() {
											...
											try {
							    				bseconn = entrydb.getBSEConnection();
							    				Boolean bsedb = entrydb.dbExists(bseconn, "BSE");
							    				if(!bsedb)
							    				{
							    					entrydb.create_bse_db(bseconn);
							    					entrydb.fill_bse_db(bseconn);
							    				}
							    				if(dataconn.getBse_Names().isEmpty())
							    				{
							    					dataconn.setBse_Names(entrydb.getName(bseconn, "BSE"));
							    					dataconn.setBse_Symbols(entrydb.getSymbol(bseconn, "BSE"));
							    				}
						    					nseconn = entrydb.getNSEConnection();
							    				Boolean nsedb = entrydb.dbExists(nseconn, "NSE");
							    				if(!nsedb)
							    				{
							    					entrydb.create_nse_db(nseconn);
							    					entrydb.fill_nse_db(nseconn);
							    				}
							    				if(dataconn.getNse_Names().isEmpty())
							    				{
								    				dataconn.setNse_Names(entrydb.getName(nseconn, "NSE"));
								    				dataconn.setNse_Symbols(entrydb.getSymbol(nseconn, "NSE"));
							    				}
							    				Thread.sleep(2000);
